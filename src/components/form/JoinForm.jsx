...
import { Form, Button, Label, InputBox, Input, InputButton, FormGroup, NameGuide, FormMessage } from '../../style/Form.style';
...
export default function JoinForm() {  
    ...
    //useForm()
    const {register, trigger, handleSubmit, getValues, formState: {errors}} = useForm({mode: "onChange"});
    const onSubmit = (data) => {
        if(isId && isEmail ){
            //회원가입 데이터 전송 api
            console.log(data);
    ...
    <Form onSubmit={handleSubmit(onSubmit)}>
        <FormGroup>
            ...
            />
            <InputButton type="button" onClick={duplicationId}>중복확인</InputButton>
            </InputBox>
            {errors.userid ? <FormMessage>{errors.userid.message}</FormMessage> : isId && <FormMessage $valid={isId}>{idText}</FormMessage>}
        </FormGroup>
        <FormGroup>
        ...
        <Button $top="28px" $buttonActive={joinActive} disabled={!joinActive} >회원가입</Button>
    </Form>
    );
}